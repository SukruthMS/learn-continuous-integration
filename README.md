...
1. What does the **runs-on** string

   Answer: The `runs-on` string specifies the environment or runner where the job will execute. In my case, it is set to `self-hosted`, meaning the workflow will run on a `self-hosted` runner machine, rather than GitHub-hosted runners. This allows greater control over the build environment and may provide access to on-premises resources.

2. In `main.yml`, on which branch do the jest tests run when a push to main branch is made?

   Answer: The Jest tests run on the `main` branch when a push is made to the `main` branch, as specified in the `on.push.branches` configuration of the workflow.

3. In `main.yml`, on which branch do the jest tests run when a pull request is submitted to the deploy branch?

   Answer: The Jest tests run on the `deploy` branch when a pull request is submitted to the `deploy` branch, as specified in the `on.pull_request.branches` configuration of the workflow.
